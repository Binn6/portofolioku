...
function LeaderboardRow({ item, type, separator }) {
  return (
    <>
      {separator && (
        <tr>
      ...
      <tr className={item.is_me ? 'bg-sql-primary/10' : 'hover:bg-surface'}>
        ...
        <td className="py-2 px-3 text-right font-mono text-sm text-sql-primary">
          {type === 'xp'
            ? <span>{(item.xp ?? 0).toLocaleString()} <span className="text-sql-dim text-xs">XP</span></span>
...
export function LeaderboardTable({ rows, callerRow, type, loggedIn }) {
  ...
  const alreadyInTop = rows.some(r => r.is_me)
  const showCallerRow = callerRow && !alreadyInTop
  ...
  return (
    <div className="overflow-y-auto flex-1">
      <table className="w-full">
        ...
        <tbody>
          {rows.map(r => <LeaderboardRow key={`${r.rank}-${r.username}`} item={r} type={type} />)}
          {showCallerRow && <LeaderboardRow item={callerRow} type={type} separator />}
        </tbody>
      </table>
      {!loggedIn && (
