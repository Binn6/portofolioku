  ...
  useEffect(() => {
    ...
    const onMove = (e) => {
      ...
      layers?.forEach((el) => {
        ...
        gsap.to(el, { x: dx * d * 14, y: dy * d * 8, duration: 1.1, ease: 'power3.out', overwrite: 'auto' })
      })
    }
  ...
  return (
    <section ref={sectionRef} className="relative min-h-screen flex items-center overflow-hidden">
      ...
      <div ref={containerRef} className="relative z-[2] max-w-6xl mx-auto px-6 w-full">
        <p data-hero data-depth="0.4" className="text-sm text-accent-muted uppercase tracking-widest mb-4 opacity-0">
          Hello, I am
        </p>
        <h1 data-hero data-depth="1.4" className="font-display text-5xl md:text-7xl font-bold text-accent mb-4 opacity-0 leading-tight">
          {profile?.name || 'Mochsabil Em Abyan'}
        </h1>
        <h2 data-hero data-depth="0.9" className="text-xl md:text-2xl text-accent-muted mb-8 opacity-0">
          {profile?.title || 'Data Analyst & Web Developer'}
        </h2>
        <div data-hero data-depth="0.5" className="flex flex-wrap gap-4 mb-12 opacity-0">
        ...
        <div data-hero data-depth="0.3" className="flex gap-4 opacity-0">
          {profile?.github && (
